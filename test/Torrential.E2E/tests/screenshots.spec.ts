...
async function addTorrentViaApi(baseURL: string, torrentPath: string) {
  ...
  const res = await fetch(`${baseURL}/api/torrents/add`, {
    method: 'POST',
...
test.describe('Screenshots', () => {
  ...
  test('torrent info pane', async ({ page }) => {
    ...
    const torrentRow = page.locator('[class*="rounded-lg border"]').filter({ hasText: 'debian' });
    await torrentRow.click();

    // Wait for the info pane to load details
