...
internal class BlockSaveService(IFileHandleProvider fileHandleProvider, TorrentMetadataCache metaCache, ILogger<BlockSaveService> logger, BitfieldManager bitfieldManager, IEventBus eventBus)
    : IBlockSaveService
{
    ...
    public async Task SaveBlock(PooledBlock block)
    {
        using (block)
        {
            ...
            await eventBus.PublishAsync(new TorrentBlockDownloaded { InfoHash = meta.InfoHash, Length = block.Buffer.Length });
            ...
            if (HasAllBlocksForPiece(blockBitfield, block.PieceIndex, chunksInThisPiece, chunksPerFullPiece))
            {
                ...
                await eventBus.PublishAsync(new PieceValidationRequest { InfoHash = block.InfoHash, PieceIndex = block.PieceIndex });
                await eventBus.PublishAsync(new TorrentPieceDownloadedEvent { InfoHash = block.InfoHash, PieceIndex = block.PieceIndex });
            }
        }
    }
