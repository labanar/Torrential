...
public sealed class PeerSwarm(
    ...
    ILoggerFactory loggerFactory) : IPeerSwarm
{
    ...
    public async Task AddToSwarm(IPeerWireConnection connection)
    {
        ...
        if (await statusCache.GetStatus(connection.InfoHash) != Data.TorrentStatus.Running)
        {
        ...
        await eventBus.PublishAsync(new PeerConnectedEvent
        {
    ...
    private async Task CleanupPeer(InfoHash infoHash, PeerId peerId)
    {
        ...
        await eventBus.PublishAsync(new PeerDisconnectedEvent
        {
            InfoHash = infoHash,
            PeerId = peerId
        });
