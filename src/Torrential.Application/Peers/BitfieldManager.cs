...
public class BitfieldManager(TorrentFileService fileService, IEventBus eventBus, TorrentMetadataCache metaCache)
{
    ...
    public async Task Initialize(TorrentMetadata meta)
    {
        ...
        for (var i = 0; i < numPieces; i++)
        {
            if (downloadBitfield.HasPiece(i) && !verificationBitfield.HasPiece(i))
                await eventBus.PublishAsync(new PieceValidationRequest { InfoHash = infoHash, PieceIndex = i });
        }
    }
