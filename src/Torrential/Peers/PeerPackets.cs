...
public static class PreparedPieceMessage
{
    public static PreparedPacket Create(int index, int begin, ReadOnlySpan<byte> data)
    {
        var pak = new PreparedPacket(13 + data.Length);
        var buffer = pak.AsSpan();
        BinaryPrimitives.WriteInt32BigEndian(buffer, 9 + data.Length);
        buffer[4] = (byte)PeerWireMessageId.Piece;
        BinaryPrimitives.WriteInt32BigEndian(buffer[5..], index);
        BinaryPrimitives.WriteInt32BigEndian(buffer[9..], begin);
        data.CopyTo(buffer[13..]);
        return pak;
    }

    public static PreparedPacket Create(ArrayPool<byte> pool, int index, int begin, ReadOnlySpan<byte> data)
    {
        var pak = new PreparedPacket(pool, 13 + data.Length);
        var buffer = pak.AsSpan();
        BinaryPrimitives.WriteInt32BigEndian(buffer, 9 + data.Length);
        buffer[4] = (byte)PeerWireMessageId.Piece;
        BinaryPrimitives.WriteInt32BigEndian(buffer[5..], index);
